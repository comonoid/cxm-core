...
- **Версия контракта**: `Client.health` читает `/health` (`hOk`/`hBackend`/`hContract`); сайт
  при маунте сверяет `hContract` с `Contract.expectedContract` и кричит о несовпадении —
  дрейф cxm-ui↔сервер ловится до первого сломанного декода. Дисциплина: любое изменение формы
  энкодера/роута на сервере = инкремент `contractVersion` там и `expectedContract` здесь.
