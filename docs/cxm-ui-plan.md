...
- Ф2.3-хвост (частично, 2026-07-07): `Client.reviseKnowledgeBy` (amount) + кнопки «▲ +50»/«▼ −50»
  в knowRow (фикс-шаг вместо ввода числа). Сервер-путь strengthen проверен curl (500→550 ✓).
  redetail-форма — не сделана (нужен ввод текста).
- **⚠ ОТКРЫТЫЙ БАГ agdelte-runtime (2026-07-07):** onClick НЕ диспатчит у кнопок, вложенных
  глубже 1 уровня (li>span>button — все ревизии; div>h2+button — Rebuild). Работают li>button
  (ростер, buy) и тулбар. Т.е. кнопки ревизий/Rebuild в браузере МЕРТВЫ (раньше «Live ✓» шёл
  curl'ом — DOM-клик не проверялся до Ф4.1-смоука). ЧИНИТЬ в agdelte (проводка event-attrs при
  билде поддерева), потом вернуть в smoke клик-тест strengthen. Репро: /tmp/dbg.mjs паттерн.
