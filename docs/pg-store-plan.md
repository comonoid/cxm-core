...
**Аудит-фиксы (2026-07-07):** (1) UNIQUE вскрыл, что `registerOwnerV` не проверял существование →
дубль-регистрация давала silent-dup (до) / unique-violation 500 (после). Добавлен find-then-reject
под login-локом → чистый **409 Conflict** (live: 2-я регистрация alice → 409, ноль исключений в логе,
txn откатывается атомарно). (2) `mIndexP` переименован в суффикс `_pidx` (не `_idx`) — чтобы hardening-
индекс не мог столкнуться с byIx-индексом на той же колонке. Оба live-проверены, pg-diff ALL GREEN.
